...
def create_analysis(validated_analysis_config):
    ...
    try:
        ...
        raise RuntimeError(f"Couldn't fetch Tool from UUID: {e}")

    analysis = Analysis.objects.create(
        uuid=str(uuid.uuid4()),
        summary=f"Galaxy workflow execution for: {tool.name}",
        name=f'{tool.get_tool_name()} - {get_aware_local_time().strftime("%Y/%m/%d %H:%M:%S")} - {tool.get_owner_username().title()}',
...
def fetch_objects_required_for_analysis(validated_analysis_config):
    ...
    try:
        ...
        raise RuntimeError(f"Couldn't fetch User from id: {user_id} {e}")
    ...
    try:
        ...
        raise RuntimeError(f"Couldn't fetch Workflow from UUID: {workflow_uuid} {e}")

    try:
        study = Study.objects.get(uuid=study_uuid)
    except (Study.DoesNotExist, Study.MultipleObjectsReturned) as e:
        raise RuntimeError(f"Couldn't fetch Study {study_uuid}: {e}")
...
def validate_analysis_config(analysis_config):
    ...
    try:
        ...
        raise RuntimeError(f"Analysis Configuration is invalid: {e}")
...
def _create_analysis_name(current_workflow):
    ...
    """
    return f'{current_workflow.name} {get_aware_local_time().strftime("%Y-%m-%d @ %H:%M:%S")}'
